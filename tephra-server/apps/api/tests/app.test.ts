...
import type { Database, GraphCacheEntry, NoteIndexRepository, Repositories, TransactionRepositories } from '@tephra/database-core';
...
class MemoryDatabase implements Database {
  ...
  graphCache = {
    entries: new Map<string, GraphCacheEntry>(),
    find: async (vaultId: string) => this.graphCacheEntries(vaultId),
    upsert: async (entry: GraphCacheEntry) => { this.graphCache.entries.set(entry.vaultId, entry); },
    deleteByVault: async (vaultId: string) => { this.graphCache.entries.delete(vaultId); },
  };
  private graphCacheEntries(vaultId: string): GraphCacheEntry | null {
    return this.graphCache.entries.get(vaultId) ?? null;
  }
  ...
  private repositories(): Repositories {
    return { users: this.users, sessions: this.sessions, vaults: this.vaults, devices: this.devices, apiTokens: this.apiTokens, blobs: this.blobs, vaultFiles: this.vaultFiles, vaultRevisions: this.vaultRevisions, fileVersions: this.fileVersions, noteIndex: this.noteIndex, graphCache: this.graphCache };
  }
}
